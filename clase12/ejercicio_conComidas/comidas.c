...
{
    printf("%04d %25s %8.2f \n",miComida.idComida ,miComida.descripcion, miComida.precio);
}
...
void mostrarComidas(eComida listaComidas[], int tamComidas)
{
   int i;
    ...
    {
        printf("%d--%s\n", listaComidas[i].idComida, listaComidas[i].descripcion);
...
int buscarLibreAlmuerzos(eAlmuerzo listaAlmuerzos[], int tamAlmuerzos)
{
    ...
    {
        if(listaAlmuerzos[i].estado != OCUPADO)
        {
            indice = i;
            break;
        }
...
int altaAlmuerzo(eAlumno listaAlumnos[], int tamAlumnos, eComida listaComidas[], int tamComidas, eAlmuerzo listaAlmuerzos[], int tamAlmuerzos, int lastIdAl)
{
    ...
    indice = buscarLibreAlmuerzos(listaComidas, tamComidas);
    ...
    {
        mostrarListadoAlumnos(listaAlumnos, tamAlumnos);

        printf("Seleccione un alumno por legajo: ");
        scanf("%d", &legajoAlumno);

        mostrarComidas(listaComidas, tamComidas);

        printf("Seleccione una comida por ID: ");
        scanf("%d", &idComida);

        printf("Ingrese la fecha de ingreso: ");
        scanf("%d/%d/%d", fecha.dia, fecha.mes, fecha.anio);

        listaAlmuerzos[indice] = newAlmuerzo(legajoAlumno, idComida, fecha, estado, lastIdAl);
        todoOk = 1;
        printf("Alta exitosa!!\n\n");
    }
...
eAlmuerzo newAlmuerzo(int legajoAlumno, int idComida, eFecha fecha, int estado, int IdAlm)
{
    ...
    almuerzo.legajoAlumno = legajoAlumno;
    almuerzo.idComida = idComida;
    almuerzo.idAlmuerzo = IdAlm;
    almuerzo.fecha = fecha;
    almuerzo.estado = 0;
